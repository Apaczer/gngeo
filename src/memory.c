...
static inline Uint16 read_neo_control(void)
{
  unsigned int scan;
  scan = current_line;
  scan += 0xF8;
  return (scan << 7) | (neogeo_frame_counter & 0x0007);
}

inline void write_neo_control(Uint16 data)
{
...
inline void write_irq2pos(Uint32 data)
{
...
/**** PALETTE ****/
/*static inline */Uint16 convert_pal(Uint16 npal)
{
  int r = 0, g = 0, b = 0;
