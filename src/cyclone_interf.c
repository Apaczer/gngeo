...
static inline void cyclone68k_store_video_word(Uint32 addr, Uint16 data)
{
  //SDL_Swap16(data);
...
static unsigned int  MyRead8(unsigned int a)
{
  ...
  case 0xC:
    if(b <= 1) {
      return (READ_BYTE_ROM(memory.rom.bios_m68k.p + addr)) & 0xFF;
    }
    break;
  case 0xd:
    if(b == 0) {
  ...
  case 0x3:
    if(b == 0xC) {
      return mem68k_fetch_video_byte(a) & 0xFF;
    }
    if(b == 0) {
      return mem68k_fetch_ctl1_byte(a) & 0xFF;
    }
    if(b == 4) {
      return mem68k_fetch_ctl2_byte(a) & 0xFF;
    }
    if(b == 8) {
      return mem68k_fetch_ctl3_byte(a) & 0xFF;
    }
    if(b == 2) {
      return mem68k_fetch_coin_byte(a) & 0xFF;
    }
    break;
  case 0x8:
    if(b == 0) {
      return mem68k_fetch_memcrd_byte(a) & 0xFF;
    }
    break;
  }
...
static unsigned int MyRead16(unsigned int a)
{
  ...
  case 0xC:
    if(b <= 1) {
      return (READ_WORD_ROM(memory.rom.bios_m68k.p + addr)) & 0xFFFF;
    }
    break;
  ...
  case 0x3:
    if(b == 0xC) {
      return mem68k_fetch_video_word(a) & 0xFFFF;
    }
    if(b == 0) {
      return mem68k_fetch_ctl1_word(a) & 0xFFFF;
    }
    if(b == 4) {
      return mem68k_fetch_ctl2_word(a) & 0xFFFF;
    }
    if(b == 8) {
      return mem68k_fetch_ctl3_word(a) & 0xFFFF;
    }
    if(b == 2) {
      return mem68k_fetch_coin_word(a) & 0xFFFF;
    }
    break;
  case 0x8:
    if(b == 0) {
      return mem68k_fetch_memcrd_word(a) & 0xFFFF;
    }
    break;
  }
...
static void MyWrite8(uint32_t a, uint32_t  d)
{
  unsigned int b = ((a & 0xF0000) >> 16);
  a &= 0xFFFFFF;
  d &= 0xFF;
  ...
  case 0x3:
    if(b == 0xc) {
      mem68k_store_video_byte(a, d);
      return;
    }
    if(b == 8) {
      mem68k_store_pd4990_byte(a, d);
      return;
    }
    if(b == 2) {
      mem68k_store_z80_byte(a, d);
      return;
    }
    if(b == 0xA) {
      mem68k_store_setting_byte(a, d);
      return;
    }
    break;
  case 0x4:
  ...
  case 0x2:
    if(b == 0xF) {
      mem68k_store_bk_normal_byte(a, d);
    }
    return;
    break;
  case 0x8:
    if(b == 0) {
      mem68k_store_memcrd_byte(a, d);
    }
    return;
    break;
  ...
  if(a == 0x300001) {
    memory.watchdog = 0;  // Watchdog
  }

  //printf("Unhandled write8  @ %08x = %02x\n",a,d);
}
static void MyWrite16(uint32_t a, uint32_t d)
{
  unsigned int b = ((a & 0xF0000) >> 16);
  a &= 0xFFFFFF;
  d &= 0xFFFF;
  ...
  case 0x3:
    if(b == 0xc) {
      mem68k_store_video_word(a, d);
      return;
    }
    if(b == 8) {
      mem68k_store_pd4990_word(a, d);
      return;
    }
    if(b == 2) {
      mem68k_store_z80_word(a, d);
      return;
    }
    if(b == 0xA) {
      mem68k_store_setting_word(a, d);
      return;
    }
    break;
  case 0x4:
  ...
  case 0x2:
    if(b == 0xF) {
      mem68k_store_bk_normal_word(a, d);
    }
    return;
    break;
  case 0x8:
    if(b == 0) {
      mem68k_store_memcrd_word(a, d);
    }
    return;
    break;
  }
...
static void MyWrite32(uint32_t a, uint32_t d)
{
  unsigned int b = ((a & 0xF0000) >> 16);
  a &= 0xFFFFFF;
  d &= 0xFFFFFFFF;
  ...
  case 0x3:
    ...
    if(b == 8) {
      mem68k_store_pd4990_long(a, d);
      return;
    }
    if(b == 2) {
      mem68k_store_z80_long(a, d);
      return;
    }
    if(b == 0xA) {
      mem68k_store_setting_long(a, d);
      return;
    }
    break;
  case 0x4:
  ...
  case 0x2:
    if(b == 0xF) {
      mem68k_store_bk_normal_long(a, d);
    }
    return;
    break;
  case 0x8:
    if(b == 0) {
      mem68k_store_memcrd_long(a, d);
    }
    return;
    break;
  }
...
void cpu_68k_mkstate(gzFile gzf, int mode)
{
  ...
  if(mode == STWRITE) {
    CyclonePack(&MyCyclone, save_buffer);
  }
  mkstate_data(gzf, save_buffer, 128, mode);
  if(mode == STREAD) {
    CycloneUnpack(&MyCyclone, save_buffer);
  }
